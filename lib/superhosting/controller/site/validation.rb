  ...
  module Controller
    class Site
      ...
      def existing_validation(name:)
        index[name].nil? ? { error: :logical_error, code: :site_does_not_exists, data: { name: name } } : {}
      end
      ...
      def not_existing_validation(name:)
        existing_validation(name: name).net_status_ok? ? { error: :logical_error, code: :site_exists, data: { name: name } } : {}
      end
